"""
PyTorch-based transformer surrogate for contracted and cored halos.

This module avoids dense grid generation by sampling the input parameter
space directly (Sobol sequence by default), evaluating the physical halo
models where needed, and training a lightweight transformer regressor for
both the contracted (Dekel) and cored SIDM halo parameters.
"""
...
class ParameterSampler:
    """Sobol sampler obeying astrophysical priors."""
...
class HaloTransformer(nn.Module):
    """Small transformer encoder with two regression heads."""
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        return self.contract_head(flattened), self.core_head(flattened)
...
def _build_dataloaders(
    ...
    scalers = {
        "x": x_scaler,
        "contra": y_contra_scaler,
        "core": y_core_scaler,
    }

    return loaders[0], loaders[1], scalers


def train_transformer_surrogate(
    *,
    n_samples: int,
...
class HaloSurrogateTorch:
    """Convenience wrapper around a trained transformer surrogate."""

    def __init__(self, bundle: Dict[str, dict], device: Optional[str] = None) -> None:
        config = bundle.get("model_config", {})
        self.model = HaloTransformer(**config)
    ...
    @classmethod
    def from_file(cls, path: str, device: Optional[str] = None) -> "HaloSurrogateTorch":
    ...
    def predict(
        ...
        contra = self.contra_scaler.inverse_transform(
            contra_scaled.cpu().numpy()
        )
...
def _cli_train(args: argparse.Namespace) -> None:
    ...
    start = time.time()
    metrics = train_transformer_surrogate(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train halo surrogate (PyTorch).")
    sub = parser.add_subparsers(dest="command", required=True)

    train_parser = sub.add_parser("train", help="Sample halos and train transformer.")
    train_parser.add_argument("--output", required=True, help="Path for torch model.")
    train_parser.add_argument("--n-samples", type=int, default=20000)
    ...
    train_parser.add_argument("--weight-core", type=float, default=1.0)
    train_parser.add_argument("--smoothness-sigma", type=float, default=0.05)
    train_parser.add_argument("--smoothness-weight", type=float, default=0.1)
    ...
    train_parser.add_argument("--log-mbar-min", type=float, default=6.5)
    train_parser.add_argument(
        "--r0-range", nargs=2, type=float, default=(0.05, 50.0)
    )
    train_parser.add_argument("--c-sigma", type=float, default=0.11)
    train_parser.add_argument("--min-delta-m", type=float, default=0.15)
